...
function GuidedTour({ steps, roleColor, onClose, currentNav, setNav, openAIKey, showToast }) {
  ...
  const speakOpenAI = async (text) => {
    if (!openAIKey) { speakFallback(text); return; }
    // Use cache to avoid re-fetching same text
    ...
    try {
      ...
      const res = await fetch("https://api.openai.com/v1/audio/speech", {
        ...
        body: JSON.stringify({
          model: "tts-1",
          input: text,
          voice: "nova",   // nova = warm, clear, professional female voice
          speed: 0.95,
        }),
      });
      ...
      setUsingAI(false);
      const isBlocked = err.message.includes("Failed to fetch") || err.message.includes("NetworkError");
      if (showToast) {
        showToast(
          isBlocked
            ? "⚠️ OpenAI voice blocked by browser security (CSP). Using built-in voice instead. To use AI voice, run the app outside Claude."
            : `⚠️ OpenAI TTS error: ${err.message}. Using built-in voice.`,
          "warn"
        );
      }
      speakFallback(text);
  ...
  const speak = (text) => {
    if (muted) return;
    stopAll();
    openAIKey ? speakOpenAI(text) : speakFallback(text);
  };
