...
export const Button = ({ children, variant = "primary", size = "md", disabled, onClick, className = "", fullWidth, style: extraStyle = {} }) => {
  const [pressed, setPressed] = useState(false);
  const variants = {
...
export const Input = ({ value, onChange, placeholder, type = "text", style: extraStyle = {} }) => (
  <input
...
export const Card = ({ children, style: extraStyle = {}, className = "" }) => (
...
export const Toggle = ({ checked, onChange, color = DS.primary }) => (
  <div
...
export const isArmed = (officer) => officer?.armed === true;
export const eventRequiresArmed = (ev) => (ev?.armedSlots || 0) > 0;
export const armedSlotsAvailable = (ev, confirmedList) => {
...
export const isSgtPlus      = (rank) => RANK_LEVEL[rank] >= 3;  // Sgt and above
export const isSpecialistPlus = (rank) => RANK_LEVEL[rank] >= 4; // Specialist and above
export const isLtPlus       = (rank) => RANK_LEVEL[rank] >= 5;  // Lt and above

export const GRACE_PERIOD_MS = 72 * 60 * 60 * 1000; // 72 hours in milliseconds
...
// ═══════════════════════════════════════════════════════════════════════════════
export function TourRoleSelector({ onSelect, onClose }) {
  return (
    <>
      <div style={{
...
// GUIDED TOUR ENGINE
// ═══════════════════════════════════════════════════════════════════════════════
export function GuidedTour({ steps, roleColor, onClose, currentNav, setNav, openAIKey, showToast }) {
  ...
  useEffect(() => {
    ...
    t2 = setTimeout(() => speak(`${clean(step.title)}. ${clean(step.body)}`), 400);
...
// NOTIFICATION DRAWER
// ═══════════════════════════════════════════════════════════════════════════════
export function NotificationDrawer({ notifications, onClose, onMarkAllRead }) {
  const typeStyles = {
...
// TOPBAR
// ═══════════════════════════════════════════════════════════════════════════════
export function TopBar({ title, officer, menuOpen, setMenuOpen, nav, setNav, notifCount, onSignOut, onBellClick, darkMode }) {
  return (
    <div style={{
...
// EVENT CARD
// ═══════════════════════════════════════════════════════════════════════════════
export function EventCard({ event, signups, onSignup, onWaitlist, onCancel, onRequestCancel, setCancelModal, isSgt, queuePos, graceLocked, graceTimeLeft }) {
...
// DASHBOARD VIEW
// ═══════════════════════════════════════════════════════════════════════════════
export function Dashboard({ officer, signups, handleSignup, handleWaitlist, handleCancel, submitCancelRequest, isSgt, showToast, startTour, events, darkMode = false, signupModal, setSignupModal, cancelModal, setCancelModal }) {
...
// SCHEDULE VIEW
// ═══════════════════════════════════════════════════════════════════════════════
export function Schedule({ signups, events, darkMode = false }) {
...
// SIMPLE VIEWS
// ═══════════════════════════════════════════════════════════════════════════════
export function SlotRelease({ showToast }) {
  return (
    <div style={{ padding: "16px 14px" }}>
...
export function FAQ({ setNav, darkMode = false }) {
...
export function Profile({ officer }) {
  return (
    <div style={{ padding: "16px 14px" }}>
...
export function Settings({ startTour, officer, openAIKey, setOpenAIKey, darkMode, setDarkMode }) {
...
export function LoginCredentials({ onNext }) {
  const [badge, setBadge]       = useState("");
  const [password, setPassword] = useState("");
  const [error, setError]       = useState("");
  const [showPass, setShowPass] = useState(false);

  const BG_IMAGE = "data:image/jpeg;base64,";
  ...
  const handleContinue = () => {
    ...
    if (password !== DEMO_PASSWORD) { setError("Incorrect password. Use DEMO1234 for demo accounts."); return; }
    onNext(officer);
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: "linear-gradient(180deg, rgba(0,0,0,0.35) 0%, rgba(0,0,0,0.55) 60%, rgba(10,14,26,1) 100%)",
          zIndex: 1,
        }} />
        ...
        <div style={{ position: "relative", zIndex: 2, marginBottom: 14 }}>
          <img
            src={SHIELD_LOGO}
            alt="CUNY Public Safety Shield"
            style={{ width: 90, height: 90, objectFit: "contain", filter: "drop-shadow(0 2px 12px rgba(0,0,0,0.7))" }}
          />
        </div>
      ...
      <div style={{
        ...
        <div style={{ marginBottom: 14 }}>
          <label style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.5, color: "#3563e9", textTransform: "uppercase", display: "block", marginBottom: 6 }}>
            Badge Number
          </label>
          <input
            type="text"
        ...
        <div style={{ marginBottom: 8 }}>
          ...
          <div style={{ position: "relative" }}>
            <input
              ...
              style={{
                width: "100%", boxSizing: "border-box",
                padding: "13px 44px 13px 14px", borderRadius: 8,
                border: "1.5px solid #dde3f0", fontSize: 15,
                outline: "none", color: "#1a1a2e",
              }}
            />
        ...
        {error && (
          <div style={{ color: "#dc2626", fontSize: 12, marginBottom: 10, textAlign: "center" }}>
            {error}
          </div>
        )}
        ...
        <div style={{
          ...
          <div style={{ fontSize: 10, color: "#8a94a6", fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 10 }}>
            Demo Accounts — Password: DEMO1234
          </div>
          {OFFICERS.map(o => (
            <button
              key={o.badge}
              onClick={() => { setBadge(o.badge); setPassword(DEMO_PASSWORD); }}
              style={{
                width: "100%", display: "flex", justifyContent: "space-between",
                alignItems: "center", padding: "10px 12px",
                background: badge === o.badge ? "#eef2ff" : "#fff",
                border: "1px solid #e8ecf4",
                borderRadius: 7, cursor: "pointer",
                marginBottom: 6, fontSize: 14, fontWeight: 600,
                color: "#1a1a2e",
              }}
            >
              <span>{o.name}</span>
              <span style={{ fontSize: 12, color: "#3563e9", fontFamily: "monospace" }}>{o.badge}</span>
            </button>
          ))}
        </div>
      </div>
...
// ═══════════════════════════════════════════════════════════════════════════════
export function LoginMFA({ officer, onVerify, onBack }) {
...
// ═══════════════════════════════════════════════════════════════════════════════
export function FirstLoginPrompt({ officer, onStartTour, onSkip }) {
  return (
    <div style={{
...
// ═══════════════════════════════════════════════════════════════════════════════
export function SignupConfirmModal({ event, officer, onConfirm, onClose }) {
...
// ═══════════════════════════════════════════════════════════════════════════════
export function CancelRequestModal({ event, onSubmit, onClose, type = "cancel" }) {
...
// ═══════════════════════════════════════════════════════════════════════════════
export function SgtApprovals({ cancelRequests, onApprove, onDeny, officer, darkMode = false }) {
...
// ═══════════════════════════════════════════════════════════════════════════════
export function RescheduleModal({ event, onConfirm, onClose, darkMode = false }) {
...
export function SupervisorDashboard({ officer, events, setEvents, confirmed, setConfirmed, notifications, addNotif, showToast, sendEmail, sendEmailToAll, cancelRequests, approveCancelRequest, denyCancelRequest, postEvent, rescheduleEvent, darkMode = false }) {
...
// ── Post Event Form ───────────────────────────────────────────────────────────
export function PostEventForm({ officer, onPost, onClose }) {
...
// ── Fire Watch Form ───────────────────────────────────────────────────────────
export function FireWatchForm({ officer, onPost, onClose }) {
...
// ═══════════════════════════════════════════════════════════════════════════════
export function AnalyticsDashboard({ events, confirmed, cancelRequests, officers, darkMode }) {
...
// ═══════════════════════════════════════════════════════════════════════════════
export function MySchedule({ officer, confirmed, events, cancelRequests, darkMode }) {
  const dm = (l, d) => darkMode ? d : l;
...
// 3. EVENT DETAIL PAGE
// ═══════════════════════════════════════════════════════════════════════════════
export function EventDetail({ event, officer, signups, onSignup, onBack, darkMode }) {
  const dm = (l, d) => darkMode ? d : l;
  if (!event) return null;
...
export function DaysOffSettings({ officer }) {
